...
def safe_clean(text: str) -> str:
    ...
    # Remove characters outside BMP to avoid weird glyphs in some terminals
...
def load_text_model(model_name: str) -> Tuple:
    ...
    try:
        ...
        try:
            ...
            raise RuntimeError(
                f"Failed to load both '{model_name}' and fallback '{FALLBACK_MODEL_NAME}'. Errors: {e} | {e2}"
            )
...
with col_right:
    ...
    max_len = st.slider("Max tokens (approx)", 32, 1024, 256, step=16)
    ...
    # Default persona - persisted in session_state
...
def build_prompt(persona: str, history, user_message: str, max_exchanges: int = 6) -> str:
    ...
    # Strong persona instruction at the start
...
def generate_response_with_transformers(prompt: str, temperature: float, top_p_val: float, max_length_val: int) -> str:
    ...
    # Transformers pipeline output may be a list of dicts
...
def generate_response(user_message: str, temperature: float, top_p_val: float, max_length_val: int) -> str:
    ...
    if current_mode == "Gemini (placeholder)":
        ...
        # Enhanced Deep Search Prompt: Provide a structured, factual answer with caution.
        deep_prompt = (
...
with col_left:
    ...
    with history_container:
        for msg in st.session_state.history:
            if msg.get("role") == "user":
                st.markdown(
                    f"<div class='msg_user'>**You:** {msg.get('content','')}</div>",
                    unsafe_allow_html=True,
                )
            else:
                st.markdown(
                    f"<div class='msg_assistant'>**{msg.get('role_label','PopKing AI')}:** {msg.get('content','')}</div>",
                    unsafe_allow_html=True,
                )
    ...
    voice_file = cols[1].file_uploader("Upload voice (wav/mp3, optional)", type=["wav", "mp3"], key="voice_upload", label_visibility="collapsed")
